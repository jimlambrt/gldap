...
func (d *Directory) handleBind(t TestingT) func(w *gldap.ResponseWriter, r *gldap.Request) {
	...
	return func(w *gldap.ResponseWriter, r *gldap.Request) {
		d.logger.Debug(op)
		resp := r.NewBindResponse(gldap.WithResponseCode(gldap.LDAPResultInvalidCredentials))
		defer func() {
			_ = w.Write(resp)
		}()
...
func (d *Directory) handleStartTLS(t TestingT) func(w *gldap.ResponseWriter, r *gldap.Request) {
	...
	return func(w *gldap.ResponseWriter, r *gldap.Request) {
		d.logger.Debug(op)
		res := r.NewExtendedResponse(gldap.WithResponseCode(gldap.LDAPResultSuccess))
		...
		if err := r.StartTLS(d.server); err != nil {
			...
			res.SetResultCode(gldap.LDAPResultOperationsError)
			w.Write(res)
			return
		}
...
func (d *Directory) handleSearchGeneric(t TestingT) func(w *gldap.ResponseWriter, r *gldap.Request) {
	...
	return func(w *gldap.ResponseWriter, r *gldap.Request) {
		d.logger.Debug(op)
		res := r.NewSearchDoneResponse(gldap.WithResponseCode(gldap.LDAPResultNoSuchObject))
		defer w.Write(res)
		...
		if len(d.tokenGroups) > 0 && strings.HasPrefix(string(m.BaseDN), "<SID=") {
			...
			res.SetResultCode(gldap.LDAPResultSuccess)
			return
		}
